...
import AgencyInventeryModel from "./AgencyInventeryModel";
import AgencyFuelEditModel from "./AgencyFuelEditModel";

const AgencyHistoryInventory: React.FC = () => {
  ...
  const fetchInventoryData = async (date: Date | null) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/inventeryAgency/getAgencyInventery?date=${dateParam}`
      );
      if (!response.ok) {
  ...
  const handleDeleteConfirm = async (itemId: string) => {
    try {
      const response = await fetch(
        `/api/inventeryAgency/deleteAgencyInventery/${itemId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const updateInventory = async (updatedData: any) => {
    ...
    try {
      const response = await fetch(
        `/api/inventeryAgency/updateAgencyInventery/${updatedData._id}`,
        {
          method: "PUT",
          headers: {
...
export default AgencyHistoryInventory;
